* Fork details
This is a fork of [[https://github.com/alphapapa/restic-runner][alphapapa/restic-runner]] that adds the following features:

+  =--config= flag for defining custom config directory outside of script
+  =restic= and =restic_opts= setting for custom restic binary locations and restic options
+  =snapshots= command mapped to =restic snapshots= output
...
2.  Make the directory =~/.config/backup/restic= (or a path of your choosing specified by the =--config= flag).
...
+  =restic=: Custom location for restic if not in =$PATH=.
+  =restic_opts=: Additional options to pass to restic.
...
+  =--config /path/to/config=: Use the name of the config directory if different from the default  =~/.config/backup/restic=.
...
Print the output of =restic snapshots=.

Example:

#+BEGIN_EXAMPLE
  # Print all snapshots for the repo
  restic-runner --repo REPONAME snapshots

  # Print snapshots for the tag configured in this set
  restic-runner --repo REPONAME --set SETNAME snapshots

  # Print snapshots for this tag
  restic-runner --repo REPONAME --tag TAG snapshots
#+END_EXAMPLE
